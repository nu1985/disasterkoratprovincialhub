...
async function main() {
    console.log('Start seeding extra data (JS version)...')
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const incident = await prisma.incident.create({
            data: {
                ...
                severity: getRandomItem(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']),
                status: getRandomItem(['NEW', 'VALIDATING', 'ASSIGNED', 'IN_PROGRESS']),
    ...
    for (let i = 0; i < 10; i++) {
        ...
        await prisma.resource.create({
            data: {
                ...
                locationId: loc.id,
                status: getRandomItem(['AVAILABLE', 'IN_USE']),
            }
        })
    }
    ...
    for (let i = 0; i < 10; i++) {
        ...
        await prisma.assignment.create({
            data: {
                ...
                status: getRandomItem(['ASSIGNED', 'ACCEPTED', 'IN_PROGRESS']),
    ...
    require('fs').writeFileSync('SEED_SUCCESS.txt', 'Verified at ' + new Date().toISOString())
}

main()
    .catch((e) => {
        console.error(e)
        require('fs').writeFileSync('SEED_ERROR.txt', e.toString())
        process.exit(1)
    })
    .finally(async () => {
